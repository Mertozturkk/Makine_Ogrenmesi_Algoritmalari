...
class LogisticRegression:
    ...
    def classify_or_prediction(self, X, w):
        return np.round(self.forward_pass(X, w))
    ...
    def fit(self, X_train, Y_train, X_test, Y_test, max_iter, learning_rate):
        print("Training..")
        p_loss = 0.0000
        for i in range(max_iter):            
            self.weights -= self.compute_gradient(X_train, Y_train, self.weights) * learning_rate
            validation_acc, validation_loss = self.evaluate(X_test, Y_test, self.weights)
            training_acc, training_loss = self.evaluate(X_train, Y_train, self.weights)
